...
ENGINE= SERVER= PORT=9090 CONNECTIONS=100 WORKERS=4 MESSAGES=100 BYTES=64 RUNS=5 PROFILE=0 OUT=
while (($#)); do
  case "$1" in
...
stop_server() {
  ...
  # setsid gives the launch command its own process group.  This also stops a
  # profiled server, where strace is the parent of the actual listening PID.
...
start_server() {
  ...
  if (( PROFILE )); then setsid bash -c "exec strace -f -c -o '$OUT/strace-run-$1.txt' $SERVER" >"$OUT/server-run-$1.log" 2>&1 &
  else setsid bash -c "exec $SERVER" >"$OUT/server-run-$1.log" 2>&1 & fi
  server_pid=$!
  for _ in {1..50}; do
...
echo 'run,engine,connections,workers,messages_per_connection,bytes_per_message,attempted,completed,bytes,elapsed_ns,throughput_msg_s,throughput_mib_s,mean_latency_us,min_latency_us,max_latency_us,server_threads' >"$OUT/summary.csv"
for run in $(seq 1 "$RUNS"); do
  ...
  awk -v r="$run" -v e="$ENGINE" -v c="$CONNECTIONS" -v w="$WORKERS" -v m="$MESSAGES" -v b="$BYTES" -v a="$attempted" -v done="$completed" -v bytes="$total_bytes" -v wall="$wall_ns" -v lsum="$latency_total" -v lmin="$latency_min" -v lmax="$latency_max" -v t="$threads" 'BEGIN {printf "%d,%s,%d,%d,%d,%d,%.0f,%.0f,%.0f,%.0f,%.3f,%.3f,%.3f,%.3f,%.3f,%d\n",r,e,c,w,m,b,a,done,bytes,wall,done/(wall/1e9),(bytes/1048576)/(wall/1e9),(lsum/done)/1000,lmin/1000,lmax/1000,t}' >>"$OUT/summary.csv"
done
