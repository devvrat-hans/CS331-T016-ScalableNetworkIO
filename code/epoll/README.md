...
- When new connections arrive, an `accept4()` loop drains up to a capped number of pending handshakes per wakeup, so a burst of new connections can't starve already-connected clients within the same `epoll_wait` cycle.
