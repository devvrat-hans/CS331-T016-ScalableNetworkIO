...
static int submit_accept(struct io_uring *ring, int server_fd) {
    ...
    if (!op) {
        fprintf(stderr, "malloc failed for ACCEPT\n");
        return -1;
    }
...
static int submit_recv(struct io_uring *ring, client_t *client) {
    ...
    if (!op) {
        fprintf(stderr, "malloc failed for RECV\n");
        return -1;
    }
...
static int submit_send(struct io_uring *ring, client_t *client) {
    ...
    if (!op) {
        fprintf(stderr, "malloc failed for SEND\n");
        return -1;
    }
...
int main(void) {
    ...
    */
    ...
    if (ret < 0) {
        fprintf(stderr,"Initial submit failed: %s\n",strerror(-ret));
    ...
    while (1) {
        ...
        if (op->type == OP_ACCEPT) {
             ...
             */
            if (submit_accept(&ring, server_fd) == -1) {
                fprintf(stderr, "Failed to submit next ACCEPT\n");
        ...
        if (op->type == OP_RECV){
            ...
            if (ret < 0) {
                fprintf(stderr, "SEND submit failed: %s\n", strerror(-ret));
        ...
        if (op->type == OP_SEND) {
            ...
            if (ret < 0) {
                fprintf(stderr, "RECV submit failed: %s\n", strerror(-ret));
